...
#
nurbsmesh = IgAShell.IGA.generate_hemisphere((NELX,NELY), ORDERS[1:2], (0.0,pi/2), (pi/10,pi/2),  R, multiplicity=(1,1))
data.grid = IgAShell.IGA.convert_to_grid_representation(nurbsmesh)

#Setsk and variables
nlayers = length(angles)
ninterfaces = nlayers - 1
maxz = maximum(getindex.(getfield.(data.grid.nodes, :x), 3))
addedgeset!(data.grid, "y-", (x) -> x[2] < 0.01)
addedgeset!(data.grid, "x-", (x) -> x[1] < 0.01)
addedgeset!(data.grid, "outer", (x) -> x[3] < 0.01)
addedgeset!(data.grid, "inner", (x) -> x[3] ≈ maxz)
addvertexset!(data.grid, "pointA", (x)-> isapprox(x[1], R, atol=1e-2) && isapprox(x[2], 0.0, atol=1e-2) && isapprox(x[3], 0.0, atol=1e-2))
pointA = [IgAShell.VertexInterfaceIndex(cellid, vertex, 2) for (cellid, vertex) in collect(getvertexset(data.grid, "pointA"))]
addvertexset!(data.grid, "pointB", (x)->  isapprox(x[1], 0.0, atol=1e-2) && isapprox(x[2], R, atol=1e-2) && isapprox(x[3], 0.0, atol=1e-2))
pointB = [IgAShell.VertexInterfaceIndex(cellid, vertex, 2) for (cellid, vertex) in collect(getvertexset(data.grid, "pointB"))]
partset1 = collect(1:(NELX*NELY))

@assert( length( getedgeset(data.grid, "y-")) == NELX)
@assert( length( getedgeset(data.grid, "x-")) == NELX )
@assert( length( getedgeset(data.grid, "outer")) == NELY )
@assert( length( getedgeset(data.grid, "inner")) == NELY )
@assert(length(pointB)==1)
@assert(length(pointA)==1)
#
...
#
#=etf = IGAShellWeakBC( 
    set = getedgeset(data.grid, "outer"),
    func = (x,t) -> (-t*1.0,), 
    comps = [3],
    igashell = igashell, 
    penalty = 1e10
...
#
etf = IGAShellWeakBC( 
    set = getedgeset(data.grid, "inner"),
...
vertexset = [IgAShell.VertexInterfaceIndex(1, 1, 1)]
etf = IGAShellWeakBC( 
    set = vertexset,
    func = (x,t) -> (0.0,), 
    comps = [3],
